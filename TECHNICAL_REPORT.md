...
- **Projection-aware folding is a static lead, not a robust live Pareto gain.**
  Fold fitting, allocation profiling, and evaluation used disjoint token
  splits. Static Pythia-410M projected-K/V PSNR rose from 34.01 to 34.82 dB;
  projection-aware direct allocation reached 34.30 dB, showing that folding
  contributed beyond layerwise precision. Fold specifications are now frozen
  as 276 KB of model metadata, closing the earlier uncounted per-sequence
  metadata gap. Across three 128-token live offsets, folded coding improved the
  geometric aggregate PPL change (+0.98% versus +2.45%) but worsened mean KL
  (0.01550 versus 0.01151) and top-1 agreement (92.71% versus 95.05%). Results
  reversed by passage. The earlier single-offset positive claim is withdrawn;
  a live serving benefit is not established.
