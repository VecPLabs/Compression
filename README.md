...
Projection-aware allocation supplies that missing objective in static tests.
Using disjoint 64-token fold-fitting, 64-token allocation-profiling, and
128-token evaluation splits on Pythia-410M, an exact per-layer allocator
improved projected-K/V PSNR from 34.01 to 34.82 dB at the nominal 3-bit budget
(4.94x versus 4.96x residual-payload compression). Projection-aware direct
allocation reached only 34.30 dB, isolating a +0.52 dB folding contribution.
The 276 KB fold specification is now fixed model metadata rather than uncounted
per-sequence state. However, three frozen 128-token autoregressive offsets do
not show uniform behavioral dominance. Folding improved aggregate PPL change
(+0.98% versus +2.45%) but worsened mean KL (0.01550 versus 0.01151) and top-1
agreement (92.71% versus 95.05%). The earlier single-offset Pareto claim is
therefore withdrawn: projection-aware folding is a static rate-distortion lead
with passage-dependent live behavior, not yet a validated serving improvement.
